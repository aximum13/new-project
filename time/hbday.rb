...
index = hbday.find_index { |i| i == today } # перебираем массив на совпадение данных: дата рождения (без года рождения) с сегодняшней датой и возвращаем индекс элемента, если он есть в массиве
index3 = hbday.find_index { |i| i == day3 } # перебираем массив на совпадение данных: дата рождения (без года рождения) с датой за 3 дня до дня рождения и возвращаем индекс элемента, если он есть в массиве
index1 = hbday.find_index { |i| i == week1 } # перебираем массив на совпадение данных: дата рождения (без года рождения) с датой за 1 неделю до дня рождения и возвращаем индекс элемента, если он есть в массиве
index2 = hbday.find_index { |i| i == week2 } # перебираем массив на совпадение данных: дата рождения (без года рождения) с датой за 2 неделю до дня рождения и возвращаем индекс элемента, если он есть в массиве

if hbday.include?(week2) #если данные "дата за 2 неделю до дня рождения" есть в массиве 
  puts "Через 2 недели (#{week2}) день рождения у #{Petrovich(firstname: hbday[index2 + 1], lastname: hbday[index2 + 2]).to(:genitive).to_s}" #В качестве значений для аргументов класс Petrovich мы находим элемент по индексу, который обнаружили ранее (индекс элемента соответствует дате рождения из файла), следующей строкой (или элементом массива) является имя сотрудника, а после него - фамилия сотрудника. Поэтому для нахождения индекса элемента "Имени" необходимо к индексу элемента "Дата" прибавить 1, соответствено для нахождения индекса элемента "Имени" к индексу элемента "Дата" прибавить 2.
end
